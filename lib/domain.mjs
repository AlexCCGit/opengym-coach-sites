...
export function createInitialState(userId) {
  const exercises = [
    ...
    { id: "plank", name: "Plancha", muscle: "core", measurement: "time", weightMode: "none" },
  ...
  return {
    schemaVersion: 1,
    userId,
    profile: { locale: "es", weightUnit: "kg", dumbbellWeightIsPerHand: true },
    exercises,
    customExercises: [],
    routines: [
      {
        ...
        exercises: [
          ...
          { exerciseId: "dumbbell-shrug", targetSets: 3, targetReps: 12 },
        ],
      },
      {
        id: "routine-core",
        ...
        color: "violet",
        exercises: [
          { exerciseId: "plank", targetSets: 3, targetSeconds: 45 },
          { exerciseId: "wall-sit", targetSets: 3, targetSeconds: 60 },
        ],
      },
    ],
    weekPlan: {
...
export function getExercise(state, exerciseId) {
  return [...state.exercises, ...state.customExercises].find((exercise) => exercise.id === exerciseId) ?? null;
}
