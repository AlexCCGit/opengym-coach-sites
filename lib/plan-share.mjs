export function buildPlanBundle(state) {
  const referenced = new Set(state.routines.flatMap((routine) => routine.exercises.map((entry) => entry.exerciseId)));
...
export function printablePlanHtml(state) {
  ...
  const cards = state.routines.map((routine) => `<section><h2>${escape(routine.name)}</h2><ol>${routine.exercises.map((entry) => `<li><strong>${escape(exercises.get(entry.exerciseId) ?? entry.exerciseId)}</strong> · ${Number(entry.targetSets ?? 3)} × ${Number(entry.targetReps ?? entry.targetSeconds ?? 0)}${entry.targetSeconds ? " s" : " reps"}${entry.supersetGroup ? ` · superserie ${escape(entry.supersetGroup)}` : ""}</li>`).join("")}</ol></section>`).join("");
