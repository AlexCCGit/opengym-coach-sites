...
export function shouldStartRest(exercises, exerciseIndex, setIndex) {
  const current = exercises?.[exerciseIndex];
  if (!current) return false;
  if (!current.supersetGroup) return true;
  const peers = (exercises ?? []).filter((entry) => entry.supersetGroup === current.supersetGroup);
  return peers.every((entry) => {
    const set = entry.sets?.[setIndex];
    return !set || entry === current || set.completed === true;
  });
}

export function workoutSetOrder(exercises = []) {
  const order = [];
  const grouped = new Set();
  for (let exerciseIndex = 0; exerciseIndex < exercises.length; exerciseIndex += 1) {
    const entry = exercises[exerciseIndex];
    if (!entry?.supersetGroup) {
      for (let setIndex = 0; setIndex < (entry?.sets?.length ?? 0); setIndex += 1) order.push({ exerciseIndex, setIndex });
      continue;
    }
    if (grouped.has(entry.supersetGroup)) continue;
    grouped.add(entry.supersetGroup);
    const peers = exercises.map((candidate, index) => ({ candidate, index })).filter(({ candidate }) => candidate.supersetGroup === entry.supersetGroup);
    const rounds = Math.max(0, ...peers.map(({ candidate }) => candidate.sets?.length ?? 0));
    for (let setIndex = 0; setIndex < rounds; setIndex += 1) {
      for (const peer of peers) if (peer.candidate.sets?.[setIndex]) order.push({ exerciseIndex: peer.index, setIndex });
    }
  }
  return order;
}

export function nextIncompleteSet(exercises, exerciseIndex, setIndex) {
  const order = workoutSetOrder(exercises);
  const current = order.findIndex((position) => position.exerciseIndex === exerciseIndex && position.setIndex === setIndex);
  const candidates = current < 0 ? order : [...order.slice(current + 1), ...order.slice(0, current)];
  return candidates.find((position) => exercises[position.exerciseIndex]?.sets?.[position.setIndex]?.completed !== true) ?? null;
}

export function recommendProgression({ policy = "off", mode = "reps", bodyweight = false, previousSets = [], targetReps = 10, minReps = Math.max(1, targetReps - 2), targetSeconds = 30, increment = 2.5, timeIncrement = 5, stalls = 0 }) {
  ...
  const deloadThreshold = policy === "greyskull" ? 1 : 3;
  if (stalls >= deloadThreshold && weight > 0) {
    return { ...result, weight: roundTo(weight * 0.9, increment), deload: true, reason: policy === "greyskull" ? "Reset Greyskull del 10 % tras un fallo" : "Descarga del 10 % tras tres estancamientos" };
  }
  if (mode === "time" || policy === "time") {
    const hit = completed.every((set) => Number(set.seconds ?? 0) >= targetSeconds);
    return { ...result, seconds: hit ? targetSeconds + timeIncrement : Math.max(seconds, targetSeconds), reason: hit ? "Objetivo temporal superado" : "Repite el objetivo temporal" };
  }
  if (policy === "double") {
    const min = Math.max(1, Number(minReps));
    const hitTop = completed.every((set) => Number(set.reps ?? 0) >= targetReps);
    const effortAllowsProgression = completed.every((set) => {
      const rir = normalizeEffort(set);
      return rir == null || rir >= 1;
    });
    const increase = hitTop && effortAllowsProgression;
    if (bodyweight && hitTop) return { ...result, weight: 0, reps: targetReps, reason: "Rango completo: elige una variante más difícil" };
    return {
      ...result,
      weight: increase ? weight + increment : weight,
      reps: increase ? min : hitTop ? targetReps : Math.max(min, reps + 1),
      reason: increase ? "Rango completo con margen: aumenta carga" : hitTop ? "Rango completo sin margen: mantén la carga" : "Progresa dentro del rango",
    };
  }
  const targetHit = completed.every((set) => Number(set.reps ?? 0) >= targetReps);
  if (weight <= 0) return { ...result, reps: targetHit ? targetReps + 1 : targetReps, reason: targetHit ? "Peso corporal: aumenta una repetición" : "Peso corporal: repite el objetivo" };
  if (policy === "greyskull") {
